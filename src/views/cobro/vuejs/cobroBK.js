...
export default {
    ...
    methods: {
        ...
        seleccionarPoliza(values) {
            console.log(values);
            var sumaSelection = 0;
            var that = this;
            that.polizas_seleccionadas = [];
            if (values.length > 0) {
                values.forEach(poliza => {
                    sumaSelection += parseFloat(poliza.importe);
                    that.polizas_seleccionadas.push(poliza.id_poliza);
                });
                that.comprobantesPago.importe = that.redondearPrecio(sumaSelection, 2);
            } else {
                that.comprobantesPago.importe = that.total_saldo;
            }
            console.log(sumaSelection);
            console.log(that.polizas_seleccionadas);
            that.operacionesMath();
        },
        clearDropzone(){
...
/*
                ...
                <v-data-table
                  ...
                  :headers="headerPolizas"
                  :items="itemPolizas"
                  @input="seleccionarPoliza($event)"
                  sort-by="id_poliza"
                  :loading="loaderPoliza"
                  item-key="id_poliza"
                  show-select
                  class="elevation-1 mt-5 mb-5"
                  loading-text="Cargando... Espere"
                  :search="buscar_poliza"
                >
                  <template v-slot:item.importe="{ item }">
                    <span>{{'$ ' + $RMT.formatoPrecio(item.importe)}}</span>
                  </template>
                </v-data-table>
*/